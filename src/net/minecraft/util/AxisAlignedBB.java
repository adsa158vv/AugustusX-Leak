...
public class AxisAlignedBB
{
    ...
    public double getLookingTargetRange(EntityPlayerSP thePlayer) {
        return getLookingTargetRange2(thePlayer, 6.0D);
    }
    public double getLookingTargetRange2(EntityPlayerSP thePlayer, double range) {
        return getLookingTargetRange3(thePlayer, null, range);
    }
    public double getLookingTargetRange3(EntityPlayerSP thePlayer, float[] rotation, double range) {
        Vec3 eyes = thePlayer.getPositionEyes(1F);
        MovingObjectPosition movingObj = calculateIntercept(eyes, (rotation != null ? RotationUtil.toDirection(rotation[0], rotation[1]) : RotationUtil.toDirection(mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch)).multiply(range).add(eyes));
        if (movingObj == null) {
