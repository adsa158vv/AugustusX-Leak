...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   public void handleJoinGame(S01PacketJoinGame packetIn) {
      ...
      this.gameController.displayGuiScreen(new GuiDownloadTerrain(Minecraft.getMinecraft().currentScreen,this));
   ...
   public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn) {
      if (mm.postDis.isToggled() && PostDis.getGrimPost()) {
         ...
         if (container != null && !packetIn.func_148888_e()) {
            if (!mm.postDis.isToggled()) {
               this.addToSendQueue(new C0FPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), true));
            } else {
               final C0FPacketConfirmTransaction c0f = new C0FPacketConfirmTransaction(packetIn.getWindowId(), packetIn.getActionNumber(), packetIn.func_148888_e());
               if (PostDis.getGrimPost()) {
                  PostDis.fixC0F(c0f);
               } else {
                  this.addToSendQueue(c0f);
               }
            }
         }
      }
   ...
   public void handleChangeGameState(S2BPacketChangeGameState packetIn) {
      ...
      if (i == 1) {
         ...
         this.clientWorldController
                 .spawnParticle(EnumParticleTypes.MOB_APPEARANCE, entityplayer.posX, entityplayer.posY, entityplayer.posZ, 0.0, 0.0, 0.0);
