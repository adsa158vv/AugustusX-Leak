...
public class OnlineConfigs extends GuiScreen {
    ...
    private final HashMap<Integer, String> loadedConfigs = new HashMap<>();
    ...
    public static void downloadJSON() throws IOException {
        URL jsonUrl = new URL("https://gitee.com/gghhddffggg/augustus-x-configs/raw/master/Menu.json");
        HttpURLConnection connection = (HttpURLConnection) jsonUrl.openConnection();
        if (connection.getResponseCode() != 200) {
            System.out.println("Access Gitee Failed! ResponseCode: "+connection.getResponseCode()+" .");
            return;
        }
        BufferedInputStream in = new BufferedInputStream(connection.getInputStream());
        FileOutputStream fileOutputStream = new FileOutputStream(Paths.get("AugustusX/onlineconfigs.json").toFile());
    ...
    private void refresh() {
        ...
        try {
            // Read the JSON file
            FileReader fileReader = new FileReader("AugustusX/onlineconfigs.json");
        ...
        if (mouseButton == 1 || mouseButton == 0) {
            for(ConfigButton configButton : this.configButtons) {
                if (this.mouseOver(
                        mouseX,
                        mouseY,
                        configButton.xPosition,
                        configButton.yPosition,
                        configButton.getButtonWidth(),
                        configButton.getHeight()
                )) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    BufferedWriter out = new BufferedWriter(new FileWriter(new File("AugustusX/configs/", loadedConfigs.get(id) + ".json")));
                    String inputLine;
                    ...
                    selected.displayString = "Error Downloading ! Code = "+responseCode+" .";
                }
            } catch (IOException e) {
                e.printStackTrace();
    ...
    private void initConfigs() {
        ...
        if (!Files.exists(Paths.get("AugustusX/configs"))) {
            try {
                Files.createDirectories(Paths.get("AugustusX/configs"));
            } catch (IOException var11) {
                var11.printStackTrace();
            }
        }

        File f = new File("AugustusX/configs");
