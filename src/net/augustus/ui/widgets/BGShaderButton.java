   ...
   public void drawButton(Minecraft mc, int mouseX, int mouseY) {
      if (this.visible) {
         ...
         if (Augustus.getInstance().getBackgroundShaderUtil().getCurrentShader().getName().equalsIgnoreCase(this.displayString)) {
            drawCenteredString(fontrenderer, this.displayString, this.xPosition + this.width / 2, y, this.getHoverColor(this.toggledColor, 0.2));
         } else {
            drawCenteredString(fontrenderer, this.displayString, this.xPosition + this.width / 2, y, this.getHoverColor(this.color, -0.2));
         }
      }
   }
