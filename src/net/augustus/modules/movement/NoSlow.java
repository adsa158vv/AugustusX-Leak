...
public class NoSlow extends Module {
    ...
    public void onEventClick(EventClick eventClick) {
        ...
        if (currentItem != null && mc.gameSettings.keyBindUseItem.pressed && (mc.thePlayer.moveForward != 0.0F || mc.thePlayer.moveStrafing != 0.0F)) {
            ...
            } else if (!(currentItem.getItem() instanceof ItemBow) && this.restBug.getBoolean() && (currentItem.getItem() instanceof ItemFood || currentItem.getItem() instanceof ItemPotion || currentItem.getItem() instanceof ItemBucketMilk) ) {
                eventClick.setShouldRightClick(false);
                if (this.counter != 3) {
                    mc.thePlayer.sendQueue.addToSendQueueDirect(new C16PacketClientStatus(C16PacketClientStatus.EnumState.OPEN_INVENTORY_ACHIEVEMENT));
                    mc.thePlayer.stopUsingItem();
