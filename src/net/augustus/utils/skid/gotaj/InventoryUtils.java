package net.augustus.utils.skid.gotaj;
...
public class InventoryUtils
{
    ...
    public static int getBlockSlot(int max)
    {
        int item = -1;
        int stacksize = 0;

        if(mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemBlock && !invalidBlocks.contains(((ItemBlock) mc.thePlayer.getHeldItem().getItem()).getBlock()) && !(mc.thePlayer.getHeldItem().stackSize <= max))return mc.thePlayer.inventory.currentItem;
        for (int i = 36; i < 45; ++i)
        {
            if (mc.thePlayer.inventoryContainer.getSlot(i).getStack() != null && mc.thePlayer.inventoryContainer.getSlot(i).getStack().getItem() instanceof ItemBlock && !invalidBlocks.contains(((ItemBlock) mc.thePlayer.inventoryContainer.getSlot(i).getStack().getItem()).getBlock()) && mc.thePlayer.inventoryContainer.getSlot(i).getStack().stackSize >= stacksize && mc.thePlayer.inventoryContainer.getSlot(i).getStack().stackSize > max)
            {
                item = i - 36;
    ...
    public static void openInv(String mode)
    {
        if (mode.equalsIgnoreCase("Spoof"))
        {
            ...
            {
            	mc.getNetHandler().getNetworkManager().sendUnregisteredPacket(new C16PacketClientStatus(EnumState.OPEN_INVENTORY_ACHIEVEMENT));
