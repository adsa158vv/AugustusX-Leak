...
public final class XylitolBlockUtil
{
    ...
    public static boolean isValidStack(final ItemStack itemStack) {
        ...
        for (final int item2 : XylitolBlockUtil.nonValidItems) {
    ...
    public static IBlockState getState(final BlockPos pos) {
        return XylitolBlockUtil.mc.theWorld.getBlockState(pos);
    }
    ...
    public static boolean isFullBlock(final BlockPos blockPos) {
        final AxisAlignedBB axisAlignedBB = getBlock(blockPos).getCollisionBoundingBox(XylitolBlockUtil.mc.theWorld, blockPos, getState(blockPos));
    ...
    public static double getCenterDistance(final BlockPos blockPos) {
        return XylitolBlockUtil.mc.thePlayer.getDistance(blockPos.getX() + 0.5, blockPos.getY() + 0.5, blockPos.getZ() + 0.5);
    }
        ...
        for (int x = radius; x > -radius; --x) {
            for (int y = radius; y > -radius; --y) {
                for (int z = radius; z > -radius; --z) {
                    final BlockPos blockPos = new BlockPos(XylitolBlockUtil.mc.thePlayer.lastTickPosX + x, XylitolBlockUtil.mc.thePlayer.lastTickPosY + y, XylitolBlockUtil.mc.thePlayer.lastTickPosZ + z);
    ...
    public static boolean collideBlock(final AxisAlignedBB axisAlignedBB, final ICollide collide) {
        for (int x = MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().minX); x < MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().maxX) + 1; ++x) {
            for (int z = MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().minZ); z < MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().maxZ) + 1; ++z) {
                final Block block = getBlock(new BlockPos(x, axisAlignedBB.minY, z));
    ...
    public static boolean collideBlockIntersects(final AxisAlignedBB axisAlignedBB, final ICollide collide) {
        for (int x = MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().minX); x < MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().maxX) + 1; ++x) {
            for (int z = MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().minZ); z < MathHelper.floor_double(XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().maxZ) + 1; ++z) {
                ...
                final Block block = getBlock(blockPos);
                if (block != null && collide.collideBlock(block)) {
                    final AxisAlignedBB boundingBox = block.getCollisionBoundingBox(XylitolBlockUtil.mc.theWorld, blockPos, getState(blockPos));
                    if (boundingBox != null && XylitolBlockUtil.mc.thePlayer.getEntityBoundingBox().intersectsWith(boundingBox)) {
                        return true;
                    }
                }
            }
