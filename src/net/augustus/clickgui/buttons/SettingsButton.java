...
import net.augustus.utils.ColorUtils;
...
public class SettingsButton extends GuiButton implements MC {
         ...
         if (!bl) {
            if (this.setting instanceof StringValue && this.isDropdownVisible()) {
               ...
               for(String options : stringValue.getStringList()) {
                  double dropY = this.yPosition + this.height * i;
   ...
   public static final ResourceLocation TURN_ON=new ResourceLocation("augx/texture/turn_on.png");
   public static final ResourceLocation TURN_OFF=new ResourceLocation("augx/texture/turn_off.png");


   private void drawCheckBox(FontRenderer fontRenderer, int mouseX, int mouseY) {
      BooleanValue setting = (BooleanValue)this.setting;
      Color colorTrue = new Color(48, 255, 162);
      Color colorFalse = new Color(45, 45, 79);
      int colorRGB;
      ...
      fontRenderer.drawStringWithShadow(setting.getBoolean()?"√":"×", (float)(this.xPosition + 5), (float)(this.yPosition + this.height / 2 - fontRenderer.FONT_HEIGHT / 2), colorRGB);
      fontRenderer.drawStringWithShadow(
         this.displayString,
         (float)(this.xPosition + 11 + fontRenderer.getStringWidth("♦")),
         (float)(this.yPosition + this.height / 2 - fontRenderer.FONT_HEIGHT / 2),
        setting.getBoolean()?colorTrue.getRGB():colorFalse.getRGB()
      );
   }

   private void drawSlider(FontRenderer fr, int mouseX, int mouseY) {
      DoubleValue setting = (DoubleValue)this.setting;
      double sliderWidth = this.width - 4;
      double sliderHeight = 10.0;
      double sliderX = this.xPosition + 2;
      double sliderY = this.yPosition + 2;
      ...
      Color sliderColor = ColorUtils.mixColors(new Color(228, 78, 166, 235),
              new Color(216, 255, 230, 235),ColorUtils.getBlendFactor(new Vector2d(this.xPosition,this.yPosition)));
      String currentValue;
      ...
      drawCenteredString(fr, text, (int)((double)((int)sliderX) + sliderWidth / 2.0), (int)(sliderY + 1.0), new Color(139, 141, 145).getRGB());
   }
   ...
   private void drawOptions(FontRenderer fr, int mouseX, int mouseY) {
      ...
      if (this.visible) {
         ...
         if (this.dropdownVisible) {
            ...
            for(String options : stringValue.getStringList()) {
               double dropY = this.yPosition + counter * this.height;
               ...
               if (options.equals(stringValue.getSelected())) {
                  stringColorRGB = this.getHoverColor2(
                     isCurrentString, this.xPosition, dropY, this.width, this.height, mouseX, mouseY, 0.2F
                  );
               } else {
                  stringColorRGB = this.getHoverColor2(
                     isNotCurrentString, this.xPosition, dropY, this.width, this.height, mouseX, mouseY, -0.2F
                  );
               }
               ...
               drawRect(this.xPosition, (int)dropY, this.xPosition +5, (int)dropY+ (double)this.height, ColorUtils.mixColors(new Color(255, 169, 251, 235),
                       new Color(216, 255, 230, 235),ColorUtils.getBlendFactor(new Vector2d(this.xPosition,(int)dropY))).getRGB());
