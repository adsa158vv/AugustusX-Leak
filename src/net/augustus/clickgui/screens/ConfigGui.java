...
public class ConfigGui extends GuiScreen {
   ...
   private void deleteConfigs() {
      try {
         ...
         if (this.selected != null) {
            File file = new File("AugustusX/configs/" + this.selected.displayString + ".json");
            boolean deleted = file.delete();
            if (deleted) {
               if (mc.thePlayer != null && mc.theWorld != null) {
                  PlayerUtil.sendChatNoSpace("§f[§6Config§f] §2Delete §8"+file.getName()+"§2 Successfully !");
               }
               System.out.println("[Config] Delete "+file.getName()+" Successfully !");
            }
            else {
               if (mc.thePlayer != null && mc.theWorld != null) {
                  PlayerUtil.sendChatNoSpace("§f[§6Config§f] §4Delete §8"+file.getName()+"§4 Failed !");
               }
               System.err.println("[Config] Delete "+file.getName()+" Failed !");
            }
         }
   ...
   private void createConfig() {
      try {
         ...
         if (!this.createTextField.getText().isEmpty()) {
