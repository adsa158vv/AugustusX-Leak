...
public final class NotificationManager {
    ...
    static {
        try {
            riseFontRenderer = new UnicodeFontRenderer(Font.createFont(0, GuiIngameHook.class.getResourceAsStream("/resources/Light.ttf")).deriveFont(18F));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void registerNotification(final String description, final NotificationType type) {
        notifications.add(new Notification(description, riseFontRenderer.getStringWidth(description) * 30L, type));
    }
    public void registerNotification(final String description) {
        notifications.add(new Notification(description, riseFontRenderer.getStringWidth(description) * 40L, NotificationType.Info));
